...
def test_get_resume_texts_on_mock_resumes(mocker):
    ...
    mock_resume_text = mocker.patch(
        "src.get_resume_texts.extract_text",
        side_effect=lambda x: f"Extracted text from {x}"
    )

    result = get_resume_texts(resume_pdfs)

    expected_result = [
        "Extracted text from resume1.pdf",
        "Extracted text from resume2.pdf"
    ]
